...
try:
    with open("physics.json", "r", encoding="utf-8") as f:
        phy_data = json.load(f)
        loaded = 0
        skipped = 0
        for q in phy_data.get("questions", []):
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/quiz 10 physics - 10 Physics questions\n"
        "/quiz 10 mole - 10 questions mole chapter se\n"
        "/biology 10 - Biology quiz\n"
        "/chemistry 10 - Chemistry quiz\n"
        "/physics 10 - Physics quiz\n\n"
        "*Chapter wise:*\n"
        "/physics 50 laws of motion\n"
        "/chemistry 20 mole concept\n"
        "/biology 10 cell\n\n"
        "*RACE commands:*\n"
...
async def chapters(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    allowed_subjects = ["Biology", "Physics"]
    if is_chemistry_allowed(chat_id):
        allowed_subjects.append("Chemistry")
    if is_race_allowed(chat_id):
        allowed_subjects.append("RACE")

    asyncio.create_task(animate_loading(context, chat_id, thread_id, "All", duration=2.5))
    await asyncio.sleep(2.6)

    chapters_set = set(
        q.get("chapter", "Unknown") for q in QUESTIONS if q.get("subject") in allowed_subjects
    )
    ...
    await update.message.reply_text(
        f"📚 *All Chapters:*\n\n{chapters_list}\n\n📝 Quiz ke liye: /quiz 10 chapter name",
        parse_mode="Markdown"
    )
...
async def biology_chapters_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    thread_id = update.message.message_thread_id
    asyncio.create_task(animate_loading(context, chat_id, thread_id, "Biology", duration=2.5))
    await asyncio.sleep(2.6)
    chapters_set = set(
        q.get("chapter", "Unknown") for q in QUESTIONS
    ...
    await update.message.reply_text(
        ...
        f"📖 Chapter wise: /biology 10 chapter name\n"
...
async def chemistry_chapters_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    thread_id = update.message.message_thread_id
    if not is_chemistry_allowed(chat_id):
        await update.message.reply_text("⛔ Chemistry is group mein allowed nahi hai.")
        return
    asyncio.create_task(animate_loading(context, chat_id, thread_id, "Chemistry", duration=2.5))
    await asyncio.sleep(2.6)
    chapters_set = set(
        q.get("chapter", "Unknown") for q in QUESTIONS if q.get("subject") == "Chemistry"
    )
    if not chapters_set:
    ...
    await update.message.reply_text(
        ...
        f"📖 Chapter wise: /chemistry 10 chapter name\n"
...
async def physics_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args or []
    if not args:
        await physics_chapters_cmd(update, context)
        return
    await run_subject_quiz(update, context, "Physics")


async def physics_chapters_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat_id = update.effective_chat.id
    thread_id = update.message.message_thread_id
    asyncio.create_task(animate_loading(context, chat_id, thread_id, "Physics", duration=2.5))
    await asyncio.sleep(2.6)
    chapters_set = set(
        q.get("chapter", "Unknown") for q in QUESTIONS if q.get("subject") == "Physics"
    )
    if not chapters_set:
        await update.message.reply_text("Physics ke questions nahi mile.")
        return
    chapters_list = "\n".join(f"• {c}" for c in sorted(chapters_set))
    if len(chapters_list) > 4000:
        chapters_list = chapters_list[:4000] + "\n...(aur bhi hain)"
...
async def race_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_race_allowed(chat_id):
        await update.message.reply_text("⛔ RACE quiz is group mein allowed nahi hai.")
        return
    args = context.args or []
    if not args:
        asyncio.create_task(animate_loading(context, chat_id, msg_thread_id, "RACE", duration=2.5))
        await asyncio.sleep(2.6)
        chapters_set = set(
            q.get("chapter", "Unknown") for q in QUESTIONS if q.get("tag") == "RACE"
        )
        if not chapters_set:
        ...
        if await send_race_quiz(chat_id, context, q, sent + 1):
            ...
            if sent < count:
                ...
                for _ in range(gap):
                    if not is_session_running(chat_id, user_id, msg_thread_id):
                        await update.message.reply_text(
                            f"🛑 RACE quiz rok diya gaya.\n📊 Total {sent} questions bheje gaye the.",
                            parse_mode="Markdown"
                        )
                        return
...
async def send_race_quiz(chat_id, context, q, question_num):
    ...
    try:
        title_msg = await context.bot.send_message(
            chat_id=chat_id,
            text=f"🏁 RACE Question #{question_num}",
            message_thread_id=thread_id,
        )
    except Exception as e:
        logger.error(f"RACE title send fail: {e}")
        return False
    await asyncio.sleep(0.25)
    try:
        await context.bot.edit_message_text(
            chat_id=chat_id,
            message_id=title_msg.message_id,
            text=f"🏁 RACE Question #{question_num} 🏎️💨🏁",
        )
    except Exception:
        pass
    await asyncio.sleep(0.25)
    try:
        ...
        try:
            await context.bot.delete_message(
                chat_id=chat_id, message_id=title_msg.message_id
            )
        except Exception:
            pass
        return True
    except Exception as e:
        logger.error(f"RACE poll send error: {e}")
        return False
...
async def quiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_chemistry_allowed(chat_id):
        ...
        if not pool:
            await update.message.reply_text("Is group mein sirf Biology aur Physics available hai.")
            return
    if count == 1:
        for _ in range(5):
            q = random.choice(pool)
...
def main():
    ...
    app.add_handler(CommandHandler("physics_chapters", physics_chapters_cmd))
