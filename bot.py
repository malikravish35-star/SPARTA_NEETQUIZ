...
async def send_race_quiz(chat_id, context, q, question_num):
    """RACE question with fast 0.5s animation, auto-delete after poll."""
    ...
    # Step 3: 0.25s baad poll + animation delete
    await asyncio.sleep(0.25)
    ...
    try:
        ...
        # Animation message delete karo
        try:
            await context.bot.delete_message(
                chat_id=chat_id,
                message_id=title_msg.message_id,
            )
            logger.info(f"RACE animation deleted: msg_id {title_msg.message_id}")
        except Exception as e:
            logger.debug(f"Animation delete skip: {e}")
